...
function render() {
    ...
    if (state.screen === "nycDinner") {
        ...
        if (
            ...
            nycDinnerStartBtn.onclick = () => {
                // Placeholder for next restaurant mini-game screen.
                nycDinnerStartBtn.disabled = true;
            };
        }
        return;
    }
    ...
    if (state.screen === "computer") {
        ...
        function startFranticLoveBugCrawl(roundId) {
            ...
            let vy = (Math.random() < 0.5 ? -1 : 1) * (420 + (Math.random() * 220));
            let lastTime = performance.now();
            ...
            function frame(now) {
                ...
                vx += (Math.random() - 0.5) * 1400 * dt;
                vy += (Math.random() - 0.5) * 1400 * dt;
